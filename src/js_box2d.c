...
static void js_draw_polygon(
    ...
    void *data)
{
    js_debug_append_polygon(
        data, b2ToRelativeTransform(transform, b2Pos_zero), vertices, vertex_count, color, false);
}
    ...
    void *data)
{
    (void)radius;
    js_debug_append_polygon(
        data, b2ToRelativeTransform(transform, b2Pos_zero), vertices, vertex_count, color, true);
}

static void js_draw_circle(b2Pos center, float radius, b2HexColor color, void *data)
{
    js_debug_append_circle(data, (b2Vec2){(float)center.x, (float)center.y}, radius, color, false);
}
    ...
    void *data)
{
    ...
    js_debug_append_circle(
        data, (b2Vec2){(float)world_center.x, (float)world_center.y}, radius, color, true);
}

static void js_draw_solid_capsule(
    b2Pos p1, b2Pos p2, float radius, b2HexColor color, void *data)
{
    b2Vec2 start = {(float)p1.x, (float)p1.y};
    b2Vec2 end = {(float)p2.x, (float)p2.y};
    js_debug_append_line(data, start, end, color);
    js_debug_append_circle(data, start, radius, color, true);
    js_debug_append_circle(data, end, radius, color, true);
}

static void js_draw_line(b2Pos p1, b2Pos p2, b2HexColor color, void *data)
{
    js_debug_append_line(
        data, (b2Vec2){(float)p1.x, (float)p1.y}, (b2Vec2){(float)p2.x, (float)p2.y}, color);
}

static void js_draw_transform(b2WorldTransform transform, void *data)
{
    b2Vec2 origin = {(float)transform.p.x, (float)transform.p.y};
    b2Vec2 x_axis = {
